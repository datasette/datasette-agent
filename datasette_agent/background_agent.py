...
async def run_background_agent(
    ...
    """Run a background agent to completion. Designed to be launched as an
    asyncio.Task. ``mode`` is the telemetry mode: ``background`` or
    ``explorer``."""
    ...
    # The whole run is one turn. create_task copied the spawning context,
    # so the span that caused the run (the API request, or the spawning
    # turn's tool span) is still current here; the run must not become
    # its child - it outlives it by minutes - so it starts a new trace
    # with a link back. Cancellation (the cancel endpoint) reaches the
    # span as a CancelledError and is recorded as outcome=cancelled.
    with turn_span(
        mode=mode, conversation_id=conversation_id, **linked_root_span_kwargs()
    ) as turn:
        turn.set_background(
            agent_id,
            max_iterations=MAX_ITERATIONS,
            spawned_from_conversation=bool(spawned_by_conversation_id),
        )
    ...
    *,
    ...
    try:
        ...
        while not finished_state["called"] and iteration < MAX_ITERATIONS:
            iteration += 1
            with background_iteration_span(iteration):
        ...
        if finished_state["called"]:
            ...
            status = "error"
        turn.set_iterations(iteration)
        turn.set_outcome("max_iterations" if not finished_state["called"] else status)
        ...
        # Cancelled: the turn span classifies it; record how far it got.
        turn.set_iterations(iteration)
        raise


async def _run_iteration(
    datasette, actor, db, turn, *, model, tools, goal, conversation_id, iteration
):
    ...
    turn.set_history(len(chain_messages))
    ...
    llm_tools = make_llm_tools(tools, datasette, actor, conversation_id=conversation_id)
    ...
    async for response in chain_response.responses():
        ...
        turn.step()
        with chat_span(model, chain_index=chain_index, streaming=False) as chat:
            chain_index += 1
